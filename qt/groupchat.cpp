    ...
    ui(new Ui::GroupChat)
{
    ...
    connect(mainWidget,&Chatlist::signals_to_sub_widget_member,this,&GroupChat::show_group_member);

    connect(mainWidget,&Chatlist::signals_to_sub_widget_group,this,&GroupChat::show_group_text);
}
...
void GroupChat::show_group_member(QJsonObject obj)
{
    if(obj.value("cmd").toString()=="get_group_member_reply")
    {
        ...
        {
            QStringList strList=obj.value("result").toString().split("|");
